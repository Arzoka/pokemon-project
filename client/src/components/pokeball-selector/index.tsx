...
const PokeballSelector: React.FC<PokeballSelectorProps> = ({
	pokeballs, currentPokeball, setCurrentPokeball, attemptingCatch, setAttemptingCatch, encounter,
}) => {
	...
	return (
		<div
			style={ {
				position: 'absolute', width: '100%', left: 0, bottom: 0, padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem',
			} }
		>
			<div style={ {
				...
				<button onClick={ () => handlePokeballChange('left') } style={ {
					width: '100px', paddingBlock: '0.5em',
				} }>
					Previous
				</button>
				{ currentPokeball ? (
					...
					>
						...
						/>
						<p style={ {
							maxWidth: '55%', wordWrap: 'break-word',
						} }
						>
